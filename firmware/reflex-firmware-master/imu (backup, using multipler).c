...
void imuInit()
{
  ...
  {
    if(writeRegisterI2C(handPorts.imu[i], I2C_MULTIPLEXER_ADDRESS, 1 << i))
    {
...
uint8_t setRegisterIMUs(uint8_t registerAddr, uint8_t data)
{
  ...
  for (int i = 0; i < NUM_IMUS; i++)
  {
    ...
    setRegisterI2C(handPorts.imu[i], BNO055_ADDRESS_A, registerAddr, data);
    resultOp = writeRegisterI2C(handPorts.imu[i], BNO055_ADDRESS_A, registerAddr);
    if (registerAddr != BNO055_SYS_TRIGGER_ADDR) // if not a reset command, check
    {
      readBytesI2C(handPorts.imu[i], BNO055_ADDRESS_A, 1, response);
...
void imu_poll_nonblocking_tick(const uint8_t imuNumber)
{
  ...
  switch(*state)
  {
    case IMU_STATE_SET_REGISTER:
      writeRegisterI2C(handPorts.imu[imuNumber], I2C_MULTIPLEXER_ADDRESS, 1 << imuNumber);
