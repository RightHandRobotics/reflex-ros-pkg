  ...
  switch (*tps)
  {
    case TPS_DONE: // initial state. kick things off.
      if (takktile_port == 2)
      {
        ...
        uint8_t msg[1] = {0x00};
        uint8_t aux[1] = {0x00};
        writeBytesSPI(handPorts.takktile[1], BCAST_ENABLE_ADDR, NULL, 0, 1);  // enable all sensors    
        udelay(sleepTime);
        writeBytesSPI(handPorts.takktile[1], BAROM_ADDR, data, 2, 1);         // send Start Conversion Sequence
        udelay(sleepTime);
        readBytesSPI(handPorts.takktile[1], BCAST_DISABLE_ADDR >> 1, 1, aux); // disable all sensors
        ...
        for (int i = 0; i < 9; i++)
        {
          ...
          writeBytesSPI(handPorts.takktile[1], addresses[i], NULL, 0, 1); // enable sensor i
          udelay(sleepTime);

          writeBytesSPI(handPorts.takktile[1], BAROM_ADDR, msg, 1, 1);       // choose register 0x00
          udelay(sleepTime);

          readBytesSPI(handPorts.takktile[1], BAROM_ADDR >> 1, 4, values);      // read 4 bytes
          udelay(sleepTime);
          ...
          readBytesSPI(handPorts.takktile[1], addresses[i] >> 1, 1, aux);
          udelay(sleepTime);
        }
      ...
      else if (takktile_port == 0)
      {
        ...
        uint8_t msg[1] = {0x00};
        uint8_t aux[1] = {0x00};
        writeBytesI2C(handPorts.takktile[0], BCAST_ENABLE_ADDR, NULL, 0, 1);  // enable all sensors    
        udelay(sleepTime);
        writeBytesI2C(handPorts.takktile[0], BAROM_ADDR, data, 2, 1);         // send Start Conversion Sequence
        udelay(sleepTime);
        readBytesI2C(handPorts.takktile[0], BCAST_DISABLE_ADDR >> 1, 1, aux); // disable all sensors
        udelay(3000);                                     // wait 3ms
        ...
        for (int i = 0; i < 9; i++)
        {
          ...
          writeBytesI2C(handPorts.takktile[0], addresses[i], NULL, 0, 1); // enable sensor i
          udelay(sleepTime);

          writeBytesI2C(handPorts.takktile[0], BAROM_ADDR, msg, 1, 1);       // choose register 0x00
          udelay(sleepTime);

          readBytesI2C(handPorts.takktile[0], BAROM_ADDR >> 1, 4, values);      // read 4 bytes
          udelay(sleepTime);
          ...
          readBytesI2C(handPorts.takktile[0], addresses[i] >> 1, 1, aux);
          udelay(sleepTime);
        }
      ...
      else if (takktile_port == 1)
      {
        ...
        uint8_t msg[1] = {0x00};
        uint8_t aux[1] = {0x00};
        writeBytesI2C(handPorts.takktile[2], BCAST_ENABLE_ADDR, NULL, 0, 1);  // enable all sensors    
        udelay(sleepTime);
        writeBytesI2C(handPorts.takktile[2], BAROM_ADDR, data, 2, 1);         // send Start Conversion Sequence
        udelay(sleepTime);
        readBytesI2C(handPorts.takktile[2], BCAST_DISABLE_ADDR >> 1, 1, aux); // disable all sensors
        udelay(3000);                                     // wait 3ms
        ...
        for (int i = 0; i < 9; i++)
        {
          ...
          writeBytesI2C(handPorts.takktile[2], addresses[i], NULL, 0, 1); // enable sensor i
          udelay(sleepTime);

          writeBytesI2C(handPorts.takktile[2], BAROM_ADDR, msg, 1, 1);       // choose register 0x00
          udelay(sleepTime);

          readBytesI2C(handPorts.takktile[2], BAROM_ADDR >> 1, 4, values);      // read 4 bytes
          udelay(sleepTime);
          ...
          readBytesI2C(handPorts.takktile[2], addresses[i] >> 1, 1, aux);
          udelay(sleepTime);
        }
