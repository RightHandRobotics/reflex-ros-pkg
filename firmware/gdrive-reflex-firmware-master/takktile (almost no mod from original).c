...
void takktile_internal_i2c_async_tick(const uint_fast8_t port)
{
  ...
  switch (*status)
  {
    ...
    case TATS_STOP:
      ...
      break;
    case TATS_STOP_WAIT:
      if (SYSTIME - g_takktile_i2c_async_start_us[port] > 10)
      {
...
void takktile_bridged_i2c_async_tick(const uint_fast8_t takktile_port)
{
  ...
  switch (*status)
  {
    case TBPS_REQUEST_CS_LOW:
      ...
      {
        spi->DR = g_takktile_i2c_async_address[takktile_port] & 0x1; // send CMD
    ...
    case TBPS_REQUEST_TX_CMD:
      ...
      {
        spi->DR = g_takktile_i2c_async_data_len[takktile_port];
...
void takktile_poll(const uint_fast8_t port)
{
  ...
  if (takktile_i2c(port, BCAST_ENABLE_ADDR, NULL, 0) != I2C_SUCCESS)
    return;
  // tell everybody we want them to start their sampling process
  uint8_t msg[4] = { 0x12, 0x01, 0x00, 0x00};
  if (takktile_i2c(port, BAROM_ADDR, msg, 2) != I2C_SUCCESS)
    return;
  // disable everybody by reading one byte...
  if (takktile_i2c(port, BCAST_DISABLE_ADDR, msg, 1) != I2C_SUCCESS)
    return;

  for (volatile uint32_t i = 0; i < 10000; i++) { } // kill some time... SO BAD

  for (uint_fast8_t sensor_idx = 0;
       sensor_idx < g_takktile_sensors_per_port[port];
       sensor_idx++)
  {
    uint8_t sensor_addr; // look up the sensor address
    if (port < NUM_FINGERS)
      sensor_addr = g_takktile_finger_addrs[sensor_idx];
    else
      sensor_addr = g_takktile_palm_addrs[sensor_idx];
    // activate this sensor
    if (takktile_i2c(port, sensor_addr, NULL, 0) != I2C_SUCCESS)
      continue;
    msg[0] = 0;
    // tell it we want to read the data
    if (takktile_i2c(port, BAROM_ADDR, msg, 1) != I2C_SUCCESS)
      continue;
    // now, actually read the data
    if (takktile_i2c(port, BAROM_ADDR | I2C_READ, msg, 4) != I2C_SUCCESS)
      continue;
