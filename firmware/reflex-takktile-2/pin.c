...
void pin_set_output_type(GPIO_TypeDef *gpio, const uint8_t pin_idx,
                                ...
                                const uint8_t function_idx)
{
  ...
  *af_reg |= function_idx << reg_ofs;   // Set the alternate function register
  ...
  gpio->MODER |= 2 << (pin_idx * 2);    // Put GPIO in alternate-function mode
}
...
void pin_set_output_level(GPIO_TypeDef *gpio, const uint8_t pin_idx, 
