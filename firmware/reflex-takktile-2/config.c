...
{  
  ...
  udelay(1000000);                 // Gives time for everything to power up
  ...
  // TODO: Be smarter. Probably doesn't need to run all the time.
  fan_on();                        
  __enable_irq();
  dmxl_set_baud_rates();
...
void printInfo(uint type)
{
  switch(type)
  {
    case HAND_STATE_INFO:
      printf("Encoders: 1:%5d 2:%5d 3:%5d \n", handState.encoders[0], 
        handState.encoders[1], handState.encoders[2]);
      ...
      for (int i = 0; i < NUM_IMUS; i++)
      {
        ...
        printf("IMU %2d: W: %6d X: %6d Y: %6d Z: %6d\n", i, handState.imus[4*i], 
          handState.imus[4*i + 1], handState.imus[4*i + 2], handState.imus[4*i + 3]);

        printf("        W: %6.2f X: %6.2f Y: %6.2f Z: %6.2f", scale*handState.imus[4*i], 
          scale*handState.imus[4*i + 1], scale*handState.imus[4*i + 2], scale*handState.imus[4*i + 3]); 
