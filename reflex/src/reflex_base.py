...
class ReFlex(object):
    def __init__(self):
        ...
        self.BLOCKED_TIME = 0.3                 # seconds before checking for blackage.
                                                # it's necessary to wait, otherwise the finger will interpret the time
                                                # spent still before movement as being blocked
        ...
        self.event_reason = [-1, -1, -1]                    # used by the event publisher to give varied error values
    ...
    def move_preshape(self, goal_pos, speed):
        if not self.hand.palm.preshape == goal_pos:
            ...
            if not self.hand.joints_publishing:
                ...
                while (abs(motor_error) > self.ARRIVAL_ERROR) and \
