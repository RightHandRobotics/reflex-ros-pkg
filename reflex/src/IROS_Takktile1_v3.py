...
## Can call... rostopic echo reflex_takktile/hand_state/finger
 ...
 ##################################################################################################################
    ...
    for i in range(125):  
	    ...
	    rospy.sleep(5.0*speedFactor)
	    pos_pub.publish(PoseCommand())  #open gripper
	    rospy.sleep(1.5*speedFactor) 
	    pos_pub.publish(PoseCommand(f1=0.0, f2=0.0, f3=0.0, preshape=1.6)) #close gripper
            rospy.sleep(1.5)
            pos_pub.publish(PoseCommand(f1=2.75, f2=2.75, f3=0.0, preshape=1.6))
            rospy.sleep(1.5)
            pos_pub.publish(PoseCommand()) # preshape in
	    rospy.sleep(1.0*speedFactor)
